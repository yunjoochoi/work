...
class DoclingParser:
    ...
    def parse(
        ...
        errors occur during parsing (only in whole-document mode).
        ...
        Args:
            ...
            is_chunk: If True, raises RuntimeError on "Invalid code point" instead of fallback

        Returns:
            List of Document objects

        Raises:
            RuntimeError: If is_chunk=True and "Invalid code point" error occurs
        """
        ...
        for result in primary_iter:
            ...
            # If this is a chunk and has critical error, raise exception for whole-document fallback
            if is_chunk and is_target_error:
                raise RuntimeError(f"Invalid code point error in chunk {filename}: {'; '.join(error_messages)}")

            # Trigger immediate fallback if critical error detected (whole-document mode only)
            if is_target_error and not is_chunk:
                try:
            ...
            elif not is_target_error:
                print(f"⚠️  [Failure] {filename} failed with error:")
    ...
    try:
        ...
        while True:
            try:
                ...
                # Determine if this is a whole-document task (chunk_index == -1)
                is_whole_document = (chunk_index == -1)
                is_chunk = not is_whole_document

                try:
                    # Process document with page offset for correct page numbering
                    file_dict = {chunk_filename: BytesIO(chunk_bytes)}
                    doc_list = parser.parse(file_dict, page_offset=page_offset, is_chunk=is_chunk)
                    ...
                    if doc_list and len(doc_list) > 0:
                        ...
                        if is_whole_document:
                            print(f"✅ [Worker-{device_str}] Whole-document {original_file_id} processed ({total_time:.2f}s)")
                    else:
                        chunk_result = ChunkResult(
                            original_file_id=original_file_id,
                            chunk_index=chunk_index,
                        ...
                        print(f"⚠️  [Worker-{device_str}] {'Whole-document' if is_whole_document else f'Chunk {chunk_index}'} of {original_file_id} failed ({total_time:.2f}s)")

                except RuntimeError as e:
                    # Check if this is an "Invalid code point" error (should only happen for chunks)
                    total_time = time.perf_counter() - start_time
                    err_msg = str(e)
                    needs_fallback = "Invalid code point" in err_msg

                    print(f"🚨 [Worker-{device_str}] Critical error in chunk {chunk_index} of {original_file_id}: {err_msg}")
                ...
                if chunks_processed >= worker_restart_interval:
                    print(f"⬇️  [Worker-{device_str}] Shutting down after {chunks_processed} chunks (restart interval reached)")
                    break

            except Empty:
                continue
            except Exception as e:
                print(f"[Worker-{device_str}] Unexpected error processing chunk: {e}")
                traceback.print_exc()
        ...
        print(f"⬇️  [Worker-{device_str}] Worker process terminated (total processed: {chunks_processed} chunks)")
...
class DocTool:
    ...
    def run(self, file_dict: Dict[str, BytesIO]) -> List[Document]:
        ...
        # Prepare tasks: chunk PDFs and add non-PDF files as whole-document tasks
        all_tasks = []
        pdf_bytes_map = {}  # Store original PDF bytes for fallback
        ...
        for filename, file_stream in file_dict.items():
            ...
            if ext == '.pdf':
                ...
                # Store original PDF bytes for potential fallback
                pdf_bytes_map[filename] = pdf_bytes
                ...
                # Non-PDF files added as whole-document tasks (chunk_index=-1)
                file_stream.seek(0)
                file_bytes = file_stream.read()

                all_tasks.append((
                    filename,        # chunk_filename (same as original)
                    -1,              # chunk_index (-1 indicates whole-document)
                    filename,        # original_file_id
                    file_bytes,      # chunk_bytes (whole file)
                    file_bytes,      # file_bytes_for_chart
                    0                # page_offset (no offset for non-PDF)
                ))
                ...
                print(f"[DocTool] Added {filename} as whole-document task")

        print(f"[DocTool] Total tasks to process: {len(all_tasks)}")
        ...
        # Track files that need full fallback
        files_to_fallback_wholly = set()
        ...
        while received_results < total_tasks:
            ...
            try:
                ...
                # Check if this file is already marked for full fallback
                if fid in files_to_fallback_wholly:
                    received_results += 1
                    if fid in doc_tracker:
                        doc_tracker[fid]["received"] += 1
                    continue  # Skip processing, just count the result

                # Check if this chunk requires full fallback
                if result.needs_full_fallback:
                    print(f"🚨 [Strategy Switch] {fid}: Critical error detected in chunk {result.chunk_index}.")
                    print(f"   -> Discarding all chunks and switching to Whole-Document Fallback.")

                    # Mark this file for full fallback
                    files_to_fallback_wholly.add(fid)

                    # Remove all previously collected chunks for this file
                ...
                if fid in doc_tracker:
                    ...
                    if tracker["received"] == tracker["total"] and fid not in files_to_fallback_wholly:
        ...
        # Process files that need whole-document fallback
        if files_to_fallback_wholly:
            print(f"\n🔄 [DocTool] Starting Whole-Document Fallback for {len(files_to_fallback_wholly)} files...")

            # Restart workers for fallback processing
            manager.start_workers()

            # Create whole-document tasks for fallback files
            fallback_tasks = []
            for fid in files_to_fallback_wholly:
                if fid in pdf_bytes_map:
                    fallback_tasks.append((
                        fid,                  # chunk_filename (same as original)
                        -1,                   # chunk_index (-1 indicates whole-document)
                        fid,                  # original_file_id
                        pdf_bytes_map[fid],   # chunk_bytes (whole PDF)
                        pdf_bytes_map[fid],   # file_bytes_for_chart
                        0                     # page_offset
                    ))
                else:
                    print(f"⚠️  [Warning] Could not find original PDF bytes for {fid}")

            # Distribute fallback tasks to queue
            ...
            fallback_results = []
            received_fallback = 0
            total_fallback = len(fallback_tasks)

            print(f"[DocTool] Waiting for {total_fallback} whole-document fallback results...")

            while received_fallback < total_fallback:
                try:
                    result = manager.result_queue.get(timeout=30)
                    fallback_results.append(result)
                    received_fallback += 1
                    print(f"[DocTool] Received fallback result {received_fallback}/{total_fallback} for {result.original_file_id}")
                except Empty:
                    continue

            # Shutdown workers again
            manager.shutdown()

            # Merge fallback results
            fallback_docs = self._merge_chunk_results(fallback_results)
            final_documents.extend(fallback_docs)
            print(f"[DocTool] Fallback processing complete. Added {len(fallback_docs)} documents.")
...
if __name__ == "__main__":
    ...
    output_root = Path("/home/shaush/work/fastfail-parsed-outputs")
