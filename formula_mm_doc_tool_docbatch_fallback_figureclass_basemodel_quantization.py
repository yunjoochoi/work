...
from docling_core.types.doc import PictureItem, TextItem, DocItemLabel
...
class DoclingParser:
    ...
    def _finalize_result(self, result, filename, raw_bytes_map, results_map, page_offset=0):
        try:
            ...
            self._remove_formula_text_items(result.document)
        ...
        for item, _ in doc.iterate_items():
            if isinstance(item, PictureItem):
                ...
                if img:
                    ...
                    #         if getattr(prov, "label", None) == DocItemLabel.FORMULA:
                    #             is_formula = True
                    #             break
    ...
    def _remove_formula_text_items(self, doc):
        """
        Remove all FORMULA TextItems from the document tree.
        This prevents broken LaTeX from appearing in Markdown.
        """
        to_remove = []

        for item, parent in doc.iterate_items():
            if isinstance(item, TextItem) and item.label == DocItemLabel.FORMULA:
                to_remove.append((item, parent))

        for item, parent in to_remove:
            try:
...
if __name__ == "__main__":
    ...
    output_root = Path("/home/shaush/work/formula-parsed-outputs")
