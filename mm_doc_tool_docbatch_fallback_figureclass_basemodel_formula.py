...
class ParserConfig(BaseModel):
    ...
    images_scale: float = 1.0  # Scale factor for generated images
...
class DoclingParser:
    ...
    # [추가] 수식 영역을 이미지로 추출하고 본문 텍스트를 이미지 링크로 교체하는 함수
    # [최종 수정] 수식 영역을 이미지로 "진짜" 추출하고 본문 교체
    def _extract_formulas_and_patch_doc(self, doc, file_key: str) -> List[Figure]:
        figures = []
        
        # [핵심 1] 로그에서 확인된 'formula' 라벨을 타겟에 추가함
        target_labels = ["formula", "display_formula", "equation", "inline_formula"] 

        print(f"[Info] Extracting formulas for {file_key}...", flush=True)

        for item, _ in doc.iterate_items():
            # 1. 라벨 가져오기
            label = getattr(item, "label", None)
            
            # [핵심 2] item.text 접근 시 안전장치 추가 (그림 파일 에러 방지)
            # 텍스트가 없는 아이템(그림 등)은 text_content가 None 또는 ""이 됨
            text_content = getattr(item, "text", "") 

            # 수식 라벨이거나, (라벨은 없는데 텍스트가 수식 패턴인 경우)
            is_target = (label in target_labels)
            
            # (옵션) 라벨이 안 잡혔는데 텍스트가 LaTeX 수식 같은 경우 강제 추출
            # 그림(PictureItem)은 text_content가 없으므로 이 if문을 안전하게 통과함
            if not is_target and text_content and isinstance(text_content, str):
                if any(x in text_content for x in ["\\frac", "\\cdot", "$$"]):
                    is_target = True
            ...
            if is_target:
                try:
                    ...
                    if page_image:
                        ...
                        scale = self.config.images_scale
                        ...
                        # 4. 이미지 크롭 (Crop)
                        cropped_img = page_image.crop(crop_rect)
                        
                        # 5. Figure 객체 생성 및 저장
                        self_ref = item.self_ref.replace("#/", "").replace("/", "_")
                        img_id = f"{file_key}/formulas/{self_ref}.png"
                        
                        figures.append(Figure(
                            id=img_id,
                            mime_type="image/png",
                            data=item._image_to_base64(cropped_img)
                        ))

                        # 6. 본문 텍스트 교체 (여기도 안전장치: text 속성이 있을 때만)
                        if hasattr(item, "text"):
                            item.text = f"![formula]({img_id})"
                        
                except Exception as e:
                    # 에러가 나도 멈추지 않고 경고만 띄우고 넘어감
                    print(f"[Warning] Failed to process formula item: {e}")
    ...
    def _process_pdf_document(self, doc, file_key: str, file_obj: BytesIO) -> Tuple[str, List[Figure]]:
        ...
        formula_figures = self._extract_formulas_and_patch_doc(doc, file_key)
        all_figures.extend(formula_figures)
...
if __name__ == "__main__":
    ...
    output_root = Path("/home/shaush/work/parsed-outputs-formula")
    ...
    # [옵션] 수식 추출 기능 켜기 (do_formula_enrichment=True)
    processor = DocTool(do_formula_enrichment=True)
    ...
    with open(log_file_path, "w", encoding="utf-8") as log_file:
        ...
        for doc in results:
            ...
            try:
                ...
                # 로그 기록
                log_msg = f"[Success] {filename} | Total Figures (Images+Formulas): {len(doc.images) if doc.images else 0}"
                print(log_msg)
                log_file.write(log_msg + "\n")
                
                # 2. 이미지 및 수식 파일 전체 저장 로직
                if doc.images:
                    saved_count = 0
                    for img in doc.images:
                        try:
                            # img.id 예시: "파일명/formulas/이미지이름.png"
                            # output_root와 결합하여 전체 경로 생성
                            image_save_path = output_root / img.id
                            
                            # 해당 이미지가 저장될 폴더 생성 (예: .../formulas/)
                            ...
                            err_log = f"   [Warning] Failed to save image {img.id}: {img_err}"
                            print(err_log)
                            log_file.write(err_log + "\n")
                    
                    log_file.write(f"   - Saved {saved_count} images to disk.\n")
