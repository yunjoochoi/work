...
class DocTool:
    """
    High-level document processing tool.
    Handles Multi-GPU dispatching internally.
    """
    def __init__(
        self,
        ...
        # 반장(Main Process)은 무거운 DoclingParser(모델 로드)를 초기화하지 않음
        # 일꾼(Worker Process)이거나 싱글 프로세스일 때만 초기화
    ...
    def _worker_process(gpu_id: int, chunk_dict: Dict[str, BytesIO], config_dict: Dict, return_queue: multiprocessing.Queue):
        """
        [Static Method] 별도 프로세스에서 실행되는 일꾼 함수
        """
        ...
        try:
            ...
            # 2. Worker용 DocTool 생성 (여기서 모델 로드됨)
            worker_tool = DocTool(
                regions_dir=config.regions_dir,
            ...
            import traceback
            traceback.print_exc()
            return_queue.put({}) # 에러 시 빈 dict 반환

    def run(self, file_dict: Dict[str, BytesIO]) -> Dict[str, str]:
        """
        Automatically distributes files across available GPUs using multiprocessing.
        """
        num_gpus = torch.cuda.device_count()
        total_files = len(file_dict)
        
        # Case 1: GPU가 없거나 파일이 적으면 그냥 현재 프로세스에서 처리 (Legacy Mode)
        if num_gpus <= 1 or total_files < num_gpus:
            if self._parser is None: # 만약 오케스트레이터로 생성됐다면 파서 초기화
        ...
        # Config를 딕셔너리로 변환하여 전달 (Pickling 용이성)
        config_dict = {
            "regions_dir": self.config.regions_dir,
...
if __name__ == "__main__":
    ...
    # 1. DocTool 초기화 (Orchestrator 모드: 메모리에 모델 안 올림)
    processor = DocTool(regions_dir=str(output_root), is_orchestrator=True)
    ...
    for filename, content in results.items():
        ...
        with open(target_dir / f"{file_stem}.md", "w", encoding="utf-8") as f:
            f.write(content)
...
#         self._parser = DoclingParser(config=config, gpu_id=gpu_id) ### 
...
# def process_file_chunk(gpu_id, file_chunk_dict, regions_dir): ###
#     """
#     각 프로세스가 실행할 함수.
#     자신에게 할당된 GPU와 파일 목록을 가지고 DocTool을 실행
#     """
#     print(f"[Process-{gpu_id}] Starting with {len(file_chunk_dict)} files on GPU {gpu_id}...")
    
#     try:
#         processor = DocTool(regions_dir=str(regions_dir), gpu_id=gpu_id)
        
#         results = processor.run(file_chunk_dict)
        
#         output_root = Path(regions_dir)
#         for filename, content in results.items():
#             file_stem = Path(filename).stem
#             target_dir = output_root / file_stem
#             target_dir.mkdir(parents=True, exist_ok=True)
...
#         print(f"[Process-{gpu_id}] Finished.")
        
#     except Exception as e:
#         print(f"[Process-{gpu_id}] Error: {e}")
#         import traceback
#         traceback.print_exc()
...
#     num_gpus = torch.cuda.device_count()
#     if num_gpus == 0:
#         print("No GPU detected. Using CPU only (Single Process).")
#         num_gpus = 1 # CPU 모드로 1개만 실행
#     else:
#         print(f"Detected {num_gpus} GPUs. Starting multi-process...")
...
#     for file_path in file_list:
#         with open(file_path, "rb") as f:
#             all_files_dict[file_path.name] = BytesIO(f.read())

#     # 파일을 GPU 개수만큼 분할 (Chunking)
#     file_names = list(all_files_dict.keys())
#     chunk_size = (total_files + num_gpus - 1) // num_gpus # 올림 나눗셈
...
#         chunk_names = file_names[start_idx:end_idx]
#         # 해당 청크의 딕셔너리 생성
#         chunk_dict = {name: all_files_dict[name] for name in chunk_names}
        
#         # 프로세스 생성 및 시작 (GPU ID 전달)
#         target_gpu = i if torch.cuda.is_available() else None
        
#         p = multiprocessing.Process(
#             target=process_file_chunk, 
#             args=(target_gpu, chunk_dict, output_root)
#         )
#         p.start()
#         processes.append(p)
